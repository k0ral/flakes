    ...
    programs = {
      ...
      git = {
        ...
        aliases = {
          ...
          co = "checkout";
          ls =
            "log --date=short --graph --pretty=format:'%Cred%h %C(bold blue)%cn%Creset %cd %C(yellow)%d %C(bold white)%s'";
          st = "! gitui";
          glog = "! tig";
          unadd = "reset HEAD";
          delete-merged = ''
          ...
          fixup =
            "! git log -n 50 --pretty=format:'%h %s' --no-merges | fzf | cut -c -7 | xargs -o git commit -p --fixup";
        };
        delta = {
          enable = true;
          options = {

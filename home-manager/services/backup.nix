...
let
  borg-paths = pkgs.writeText "borg_paths" ''
  ...
  backupScript = pkgs.writeShellScriptBin "backup" ''
    ...
    ${pkgs.borgbackup}/bin/borg create --paths-from-stdin --remote-path=borg1 -x -C lzma "$REPOSITORY:backup::{utcnow}-{hostname}" < ${borg-paths}
